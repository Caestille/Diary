...
namespace Diary.Core.Extensions
{
    internal static class DiaryDayViewModelExtensions
    {
        ...
        {
            return new DiaryDayDto() { Name = viewModel.Name, Entries = viewModel.ChildViewModels.Select(x => (x as DiaryEntryViewModel).ToDto()).ToList(), DayOfWeek = viewModel.DayOfWeek };
        }
    }
}
